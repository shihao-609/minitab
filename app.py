...
def _records_time_range(records: list, key: str) -> str:
    """从记录列表的指定日期/时间字段提取最早/最晚，返回 'YYYY-MM-DD ~ YYYY-MM-DD'。

    支持 'YYYY-MM-DD' 纯日期与 'YYYY-MM-DD HH:MM[:SS]' 两种格式；字段缺失或空值跳过；
    取前 19 字符并按字典序 min/max（ISO 格式天然有序），无有效值返回 ''。
    """
    times = []
    for r in records or []:
        ...
        if len(v) == 10:          # 'YYYY-MM-DD'
            times.append(v)
        elif len(v) >= 16:        # 'YYYY-MM-DD HH:MM[:SS]'
            times.append(v[:16])
    if not times:
        return ''
...
def _render_submission_tab(inspect_type):
    ...
    if st.session_state.get('_sub_active_type') != inspect_type:
        ...
        for k in ['_sub_fid', '_sub_type', '_sub_df', '_sub_preview', '_sub_import_result', '_sub_clean_filled']:
    ...
    if uploaded is not None:
        ...
        if st.session_state.get('_sub_fid') != fid or st.session_state.get('_sub_type') != inspect_type:
            ...
            try:
                clean_report = {}
                df = inspection_match.parse_sheet(uploaded, 'submission', clean_report=clean_report)
            except Exception as e:
    ...
    trange = _records_time_range(records, key='received_date')
    st.caption(f'📅 收料日期范围（已加载 {len(records)} 条）：**{trange}**' if trange
               else f'📅 收料日期范围：未知（已加载 {len(records)} 条）')
...
def _render_inspection_records_tab(inspect_type):
    ...
    if not records:
        st.info('暂无检验记录')
        return
    trange = _records_time_range(records, key='inspect_date')
    st.caption(f'📅 质检日期范围（已加载 {len(records)} 条）：**{trange}**' if trange
