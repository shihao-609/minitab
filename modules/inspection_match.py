...
================================
功能：
  1. 解析送检清单 Excel（6 列：供应商 / 物料编码 / 规格型号 / 物料名称 / 收料日期 / 实收数量）
  2. 解析检验清单 Excel（6 列：供应商 / 物料编码 / 规格型号 / 物料名称 / 质检日期 / 检验数量）
...
def _map_columns(df, kind):
    ...
    remap = {}
    used = set()
    df_cols = [_clean_text(c) for c in df.columns]
    for std in target:
